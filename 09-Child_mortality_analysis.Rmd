<<<<<<< HEAD:09-Child_mortality_analysis.Rmd
...
We shall now analyze a few major causes of child mortality and how these causes have affected different parts of the
```{r, fig.width=8, fig.height=8, comment = NA, echo=FALSE, message = FALSE, warning = FALSE}
...
dt <- df[df$Entity %in% countries,] %>% rename(Region = Entity) 
...
We have used D3 to create a time series choropleth map for various countries in the world. We had received the country wise mortality rate and generated a mapping from country to country-code. We use the country codes and the mortality rates for 1950 to 2015 to construct the time series. Hit the play button to start the time-series from the year 1950. During the timelapse we notice a lot of changes in the African and Asian continent which shows that there have been a lot of events that have affected the well being of children in this areas. The continents as a whole tend get lighter in shade over time , which shows that the death rate is lowering gradually.

<iframe src="Mortality_rate_time_series.html" width="1000" height="700"></iframe>
...
df_spread<-spread(df, Year, mortality_rate)
df_spread <- df_spread %>% rename(id = Code)
...
>>>>>>> 6eabdababff5329e72032f36ced4c22538bd890a:Child_mortality_analysis.Rmd
